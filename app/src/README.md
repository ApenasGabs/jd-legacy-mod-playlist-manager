...
- Use Nuitka to compile the application into a high-performance standalone C++ binary.
- This setup bundles all external tools (xtx_extract.exe), assets, and UI files into a single file.
- IMPORTANT: The command MUST be executed from the 'app' folder.
...
2. Run the compilation command (PowerShell):
> This command maps the 'src' and 'resources' directories to match your project's internal logic.

```powershell
# 1. Set PYTHONPATH to 'src' so Nuitka resolves 'config', 'core', 'ui', etc.
$env:PYTHONPATH="src"
...
- Bundled Tools: The --include-data-dir=resources=resources flag ensures that xtx_extract.exe and config_ipk_packer.json are available inside the executable environment.
- Module Mapping: Since main.py relies on src being in the system path, the $env:PYTHONPATH="src" prefix is mandatory for the compiler to see your code structure.
- UI Loading: The application will look for .ui files in resources/gui/. Ensure your code references these using paths relative to the entry point.
- Dynamic Folders: Directories like output/ or patch_nx_backups/ are excluded from the build as they are generated by the app during execution.

---

## Security & Integrity Verification

Due to the lack of a digital signature, antivirus programs may flag the .exe as a false positive. Verify the file's authenticity using the SHA-256 Checksum.
...
| File | SHA-256 Hash |
| :--- | :--- |
| JD2022LMPlaylistManager.zip | PASTE_YOUR_HASH_HERE |
...
- Create a ZIP archive that includes:
    - The generated `JD2022LMPlaylistManager.exe`.
    - The `runtime/songs.json` file (required for the first run).
- The application's logic will automatically recreate all other necessary folders (output/, patch_nx_backups/, runtime/) on the user's machine upon first launch.